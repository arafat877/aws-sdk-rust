...
if let Some(hardware_type) = crate::endpoint_lib::substring::substring(bucket
,49
,50
,true, _diagnostic_collector) { #[allow(unused_variables)]
if let Some(region_prefix) = crate::endpoint_lib::substring::substring(bucket
,8
,12
,true, _diagnostic_collector) { #[allow(unused_variables)]
if let Some(abba_suffix) = crate::endpoint_lib::substring::substring(bucket
,0
,7
,true, _diagnostic_collector) { #[allow(unused_variables)]
if let Some(outpost_id) = crate::endpoint_lib::substring::substring(bucket
,32
,49
,true, _diagnostic_collector) { #[allow(unused_variables)]
if let Some(region_partition) = partition_resolver.resolve_partition(region, _diagnostic_collector) { if (abba_suffix) == ("--op-s3") {
...
return Err(aws_smithy_http::endpoint::ResolveEndpointError::message("Path-style addressing cannot be used with FIPS"
.to_string()));
                            }
